...
[numhid numpen]=size(hidpen);
 ...
 %temp_h1=temp_h1>rand(size(temp_h1));
 
 temp_h2 = 1./(1 + exp(-temp_h1*hidpen - bias_pen));

 %temp_h2=temp_h2>rand(size(temp_h2));
 ...
 for ii= 1:10 % Number of the mean-field updates. I also used 30 MF updates.  
   ...
   %if ii<20
   % temp_h1_new=temp_h1_new>rand(size(temp_h1_new));
   %end   
   ...
   %if ii<20
   % temp_h2_new=temp_h2_new>rand(size(temp_h2_new));
   %end   
